...
import sys
import os
from application.models import Ticket, Response, FAQ, User
from flask_sqlalchemy import SQLAlchemy
db = SQLAlchemy()

BASE="http://127.0.0.1:5000" 
url_login = BASE+"/sitaram"
...
url_Sitaram = '/api/sitaram_user'
url_Registration = '/api/register'
url_Verification = '/api/self_account/activate'
url_Discourse_post = '/api/sitaram_post1'
...
#post request for Category_API
valid_token_here="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJleHAiOjE3MTIyNTMzNjN9.sD9YOi8rt6mqOYWYTQOxIEb0BLaYjKfMS_LfsVLmFb4"
...
# get request for sitaram_user APi

def test_get_Sitaram_unauthenticated():
    request = requests.get(url_Sitaram)
    response = request.json()
    assert request.status_code == 200
    assert response['status'] == 'unsuccessful, missing the authtoken'

def test_get_Sitaram_authenticated():
    header = {"secret_authtoken": "abcxyz", "Content-Type": "application/json"}
    request = requests.get(url=url_Sitaram, headers=header)
    response = request.json()
    assert request.status_code == 200

#get request for Registration API

def test_get_Registration_authenticated():
    # Assuming you have a valid token for an authenticated user
    header = {"secret_authtoken": valid_token_here}
    request = requests.get(url=url_Registration, headers=header)
    response = request.json()
    assert request.status_code == 200
    # Add assertions to check the response content

def test_get_Registration_unauthenticated():
    request = requests.get(url=url_Registration)
    response = request.json()
    assert request.status_code == 200
    # Add assertions to check the response content
...
def test_post_Registration_success():
    data = {
        "email": "005ajeet@gmail.com",
        "password": "12345"
    }
    request = requests.post(url=url_Registration, json=data)
    response = request.json()
    assert request.status_code == 200
 

def test_post_Registration_existing_user():
    data = {
        "email": "005ajeet@gmail.com",
        "password": "12345"
    }
...
#get request for Verification API

def test_get_Verification_activate():
    # Assuming the user with the email "005ajeet@gmail.com" is initially inactive (status=0)
    email = "005ajeet@gmail.com"
    user1 = User(email_id=email, status=1)  # Created a new user with status=0
    db.session.add(user1)
    db.session.commit()
    header = {"secret_authtoken": valid_token_here}
    request = requests.get(url=url_Verification, headers=header)
    response = request.json()
    assert request.status_code == 200
    assert response["message"] == "activated"
    # Add assertions to check the user's status in the database

def test_get_Verification_already_activated():
    # Assuming the user with the email "005ajeet@gmail.com" is initially active (status=1)
    email = "005ajeet@gmail.com"
    user1 = User(email_id=email, status=1)  # Create a new user with status=1
    db.session.add(user1)
    db.session.commit()
    ...
    response = request.json()
    assert request.status_code == 200
    assert response["message"] == "already activate"
    # Add assertions to check the user's status in the database

#get request for DiscoursePost API

def test_get_Discourse_post_successful():
    header = {"secret_authtoken": valid_token_here}
    request = requests.get(url=url_Discourse_post, headers=header)
    response = request.json()
    assert request.status_code == 200
    assert response["message"] == "POST request successful"
    # Add assertions to check the response content and status

def test_get_Discourse_post_failed():
    header = {"secret_authtoken": valid_token_here}
    request = requests.get(url=url_Discourse_post, headers=header)
    response = request.json()
    assert request.status_code == 500
    assert response["message"] == "POST request failed"
    # Add assertions to check the response content and status